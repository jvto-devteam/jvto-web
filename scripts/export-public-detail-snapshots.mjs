...
async function serializePackageDetail(pkg) {
...
try {
  ...
  for (const pkg of packages) {
    packageItems.push({
      ...
      payload: await serializePackageDetail(pkg),
    });
  }
  ...
  console.log(
    `Wrote ${packageItems.length} package detail snapshots to ${PACKAGE_OUTPUT_PATH}`,
  );
  console.log(
