...
**Last completed:** Cross-repo consistency fix + design-reference cleanup (2026-08-03). A design-session audit flagged the fact-layer as drifted; verification showed **production was already correct** and the real root was upstream — llm-wiki's compilers still *enforced* the retracted (pre-2026-07-06) rules, regenerating drift on every recompile. Fixed at the root across all three repos — 4 PRs, all merged (`verify` green on each):
...
**Open items:**
- **2016 incorporation dropped everywhere (2026-08-03)** — now guarded in all three repos (`INCORPORATION-2016` in llm-wiki `claim_boundaries.yml` + OKF `publication-rules.yaml`; hardened `wrong-founding-year` in jvto-web `contentDriftRules.mjs`). Reinstate as *evidenced* **only** if the owner files a verifiable AHU incorporation document.
