...
**Last completed:** Cross-repo fact-correction pass using llm-wiki + knowledge-catalog-jvto-bootstrap (OKF) as sources, closing most of the `priority_order_canonical.md` pre-PDF blockers this file had listed as open since 2026-05-26. Key findings and fixes:
...
**Build status:** `npm run validate` (schema + routes + content-drift + itinerary + OKF consumption) PASS. `npm run lint:gate` PASS (131 errors, at/under the 131-error baseline). `npx tsc --noEmit`: 2 pre-existing errors only (see above), zero from this session's changes. `npm run build` **not run** — no `DATABASE_URL`/`.env` available in this environment (SSG build needs Postgres); do not treat this as a build failure, it was never attempted. Content-drift baseline updated 55 hits/23 buckets → 19 hits/14 buckets via `--update`, reflecting real fixes.
**Next task:** Owner direction needed. Two open paths remain, same framing as before this sprint but with the pre-PDF blockers now cleared: (A) **Phase 2 of PDF plan** (`~/.claude/plans/sementara-ini-aku-butuh-tingly-diffie.md`) — public email-gated download feature with NextAuth magic-link verify + Nodemailer attachment delivery + new `pdf_downloads` Prisma model. (B) New open items from this sprint (below) — mostly DB/credential-shaped, needing owner or DB access this session didn't have.
**Open items:**
- KTA card identifiers have no home in the DB schema (`crew_members` has no `kta_id` column) — the `/team` page fix this sprint is an in-process lookup stopgap (`crewMembers.ts`), not a real fix. Needs a migration + populate + `crewMembers.ts` select update once DB access is available.
- `reviewApiSnapshots.json` (`src/lib/publicContent/generated/`) — `stats` block is correct (51/123/21/195) but the `feed` array (156 items) doesn't sum to `stats.total` (195) and undercounts Trustpilot/overcounts Google relative to the correct split. Regenerate via `scripts/export-public-review-api-snapshots.mjs` (needs `DATABASE_URL`) — do not hand-edit.
- OKF sync pipeline needs the `JVTO_WEB_DISPATCH_TOKEN` secret configured on `knowledge-catalog-jvto-bootstrap` (mirrors the one llm-wiki already uses) before its `repository_dispatch` notify step actually fires — until then `sync-okf.yml` still works via manual `workflow_dispatch`.
