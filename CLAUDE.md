...
**Last completed:** Workstream C of `~/.claude/plans/modular-kindling-clock.md` — DB-driven content ingest of Guardian Archetype + Trust-Pillar Crew Mapping into `/why-jvto/*`. Prepended `guardian-archetype` section to `/why-jvto/our-story` (row id=16) and `guardian-mindset` + `trust-pillar-crew-mapping` sections to `/why-jvto/our-team` (row id=17). Sources verbatim from llm-wiki SSOT: `wiki/content/copy-bank.md:93-99` + `wiki/people/crew-registry.md:129-163`. Mechanism: new helper `scripts/ingest-workstream-c.mjs` (Prisma upsert, dry-run by default, `INGEST_CONFIRM=1` to apply, idempotency guard) → snapshot regen → build. Block strategy: single `markdown` block per section, GFM syntax (`>` blockquote + `|---|` table) — renders via canonical `src/components/content/MarkdownRenderer.tsx` (react-markdown + remark-gfm). Mid-implementation correction: original plan locked "no GFM" constraint based on subagent reading dead-code file at `src/components/website/MarkdownRenderer.tsx`; canonical renderer IS GFM-enabled — owner re-approved revised approach. Commits: `8eedb1a` (chore: ingest script), `6824b00` (feat: content). Wiki log entry committed in llm-wiki as `b4ffd9b`. Plan file: `~/.claude/plans/phase-start-workstream-golden-wilkinson.md`.
**Completed date:** 2026-05-25
**Build status:** ✓ Compiled successfully in 12.7s + ✓ Generating static pages (143/143) in 4.2s.
**Next task:** Workstream B — EAV NLP sharpening across 8+ schema/Faqs files. Two parallel claim threads: (1) `operator.legalName` "JVTO" → `"PT Java Volcano Rendezvous, NIB 1102230032918"` wherever surfaces in `legalName` / description fields (not display/marketing copy); (2) Ijen `healthScreening` mentions → canonical `"BBKSDA SE.1658/KSA.9/2024 — supervised by Dr. Ahmad Irwandanu SIP (Kemenkes/KKI)"` per `wiki/content/brand-voice.md:60-66`. Files to grep + patch: `src/lib/schemas/build*.ts` (8 files) + relevant `src/lib/*Faqs.ts`. Parallel `narrative_claims` row edits in DB for C1/C4/C5 to prevent resolver-precedence rollback. Per parent plan §Workstream B, split per file group (schemas vs Faqs) for reviewable commits. Verification: `rg "mandatory health screening" src/` → 0 hits after; `rg "jobTitle.*Founder" src/lib/schemas/` → 0 hits (FOUNDER context only).
**Open items:**
