...
Both `main` and `live` deploy through the **same** `.github/workflows/deploy.yml` ("Deploy to VPS"), checked out per-branch. `README.md` is the (secret-free) ops runbook.
...
- **Mechanism:** `appleboy/ssh-action` SSHes to the Hostinger VPS (`31.97.223.43`) and runs: source nvm → capture `PM2_BIN` → **`nvm use 20`** (Next.js 16 needs Node ≥20.9; box default is Node 18) → `git reset --hard origin/main` → `npm ci` → `npm run build` → `pm2 restart jvto-help --update-env`. Not Vercel/Docker. **Never add `git clean`** (it would delete the VPS-local `.env.local`). Any manual on-box command must mirror the `nvm use 20` step or the build aborts on Node 18.
