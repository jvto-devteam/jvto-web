...
- **Sebelum push apa pun, periksa commit tertahan: `git log --oneline origin/live..HEAD`.** Commit dokumen yang menunggu tumpangan WAJIB ikut di push yang sama. Menahannya adalah penghematan hanya selama ada pekerjaan kode berikutnya yang datang; begitu ia menumpuk lebih dari satu siklus tanpa alasan baru, itu bukan efisiensi lagi melainkan catatan yang tidak tayang. Kalau tidak ada pekerjaan kode yang akan datang, push dokumennya sendiri dan bayar siklusnya — lebih murah daripada `STATUS.yaml` yang menyimpang dari apa yang benar-benar berjalan.
