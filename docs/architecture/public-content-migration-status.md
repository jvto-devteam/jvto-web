...
| 07 | Sitemap lastmod: remove the `content_pages` (Prisma) fallback; enumeration unchanged | **IMPLEMENTED · PREVIEW-VERIFIED** (#164, merged `32faf46d`) — lastmod resolves content/ → snapshot → explicit static date, and `getLastModified` resolves on demand so correctness never depends on a hand list; `test:sitemap-lastmod` covers all **56 emitted** routes + proves the path is DB-free. Preview proof: help `build-info.commitSha` == `32faf46d`, `/sitemap.xml` (72 `<url>`) returns byte-identical `lastmod` across two fetches 5s apart, and no `lastmod` is within 10 minutes of now — i.e. no request-time leak |
| M2-REMAINING | **Milestone 2 — remaining public narrative cutover**: `/`, `/trust`, `/contact`, `/markets/{singapore,malaysia}`, `/isic/student-package`, `/tours`, `/tours/from-bali`, `/tours/from-surabaya` → `content/` | **IMPLEMENTED · PREVIEW-VERIFIED** (#165, merged `ae8e4534`) — **9 of the 10 candidate routes** are content-owned (evergreen narrative + SEO + canonical + FAQ); DB keeps package/price/availability, reviews, destination/operational state, booking/checkout; snapshot + CMS-seed rows removed for the migrated set; `test:m2-narrative` route-parity gate added. **`/student-deals/isic` was deliberately NOT migrated**: `next.config.ts` redirects it `permanent: true` → `/isic/student-package` (and `middleware.ts` maps it too), so the URL never renders — publishing it would have put an unreachable duplicate in the feed as a canonical page. `validate-public-knowledge.mjs` now **fails any compiled route that is a redirect source**, so this cannot recur. Ownership registry: **47 → 56** content routes (delta **+9**, counted from `listPublishedStaticPages()`, not assumed). Preview proof (help @ `ae8e4534`, 2026-08-09): `build-info.commitSha` == `knowledge-feed.sourceCommit` == `ae8e4534`, feed **56 routes**, `/` + `/tours` + `/markets/singapore` + `/isic/student-package` + `/contact` each HTTP 200 with the **production** canonical, help still `noindex, nofollow`. **Two carry-forwards left open by owner decision** (no PR): the dead `/student-deals/isic` component's stale link, and the dormant homepage seed narrative |
| 11a | **Public web design refactor — shared foundation + homepage**: refactor the homepage visual layer on the now-stable content/ narrative + dynamic data contracts; shared design tokens/components only where the homepage actually uses them | **IMPLEMENTED · PREVIEW-VERIFIED** (#166, merged `905988f6`) — visual layer only. Copy/facts, URLs + production canonicals, FAQ-visible == FAQPage, metadata + JSON-LD, dynamic destination/tour/package/price reads, and booking/checkout are all unchanged; no authority moves and no business logic changes. Later clusters are deliberately **not** redesigned in this PR. Preview proof (help @ `905988f6`): `build-info.commitSha` == `knowledge-feed.sourceCommit` == `905988f6`, feed **56 routes**, `/` + `/tours` + `/tours/from-bali` + `/destinations` + `/travel-guide/faq` + `/why-jvto` each HTTP 200 with **zero redirects**, homepage canonical on the production origin, exactly **one** FAQPage with all 6 `mainEntity` questions visible, help still `noindex, nofollow`, and the design layer live in the served HTML/CSS (`font-jvto-heading`, `jvto-focus`, `jvto-topo`, `--color-jvto-navy-deep`, `--color-jvto-lime-ink`, `--color-jvto-on-navy`). **Open, owner-gated:** `HomeTravelGuideTeaser` still has no photograph — the slot renders as a captioned plate; per owner decision (2026-08-09) it stays until a real JVTO photo exists (no stock, no generated image) |
| 11b | **Tours discovery design**: `/tours`, `/tours/from-bali`, `/tours/from-surabaya` — apply the 11a shared foundation; improve hierarchy, scanning, route differentiation, mobile usability, conversion path | **IN PROGRESS (this PR)** — visual layer only. Packages, prices, availability, filters and card data stay DB-sourced (`getPublicPackageList`); no change to pricing logic, checkout, booking, route URLs, canonicals, metadata or JSON-LD, and no copy rewritten. `TourCard` is deliberately **untouched** — it has 8 consumers (homepage, destinations, ISIC, carousels), so restyling it would leak this redesign into out-of-scope clusters |
| 10, 11c+ | per blueprint | pending, one PR each |
