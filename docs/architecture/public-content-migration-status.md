...
| M2-REMAINING | **Milestone 2 — remaining public narrative cutover**: `/`, `/trust`, `/contact`, `/markets/{singapore,malaysia}`, `/student-deals/isic`, `/isic/student-package`, `/tours`, `/tours/from-bali`, `/tours/from-surabaya` → `content/` | **IN PROGRESS (this PR)** — evergreen narrative + SEO + canonical + FAQ are content-owned for all 10; DB keeps package/price/availability, reviews, destination/operational state, booking/checkout; snapshot + CMS-seed rows removed for the 10; `test:m2-narrative` route-parity gate added. Ownership registry: **47 → 57** content routes (delta **+10**, counted from `listPublishedStaticPages()`, not assumed) |
