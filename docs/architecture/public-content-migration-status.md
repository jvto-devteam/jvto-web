...
| 11a | **Public web design refactor — shared foundation + homepage**: refactor the homepage visual layer on the now-stable content/ narrative + dynamic data contracts; shared design tokens/components only where the homepage actually uses them | **IN PROGRESS (this PR)** — visual layer only. Copy/facts, URLs + production canonicals, FAQ-visible == FAQPage, metadata + JSON-LD, dynamic destination/tour/package/price reads, and booking/checkout are all unchanged; no authority moves and no business logic changes. Later clusters are deliberately **not** redesigned in this PR |
| 10, 11b+ | per blueprint | pending, one PR each |
