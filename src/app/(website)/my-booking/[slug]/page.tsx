...
import { MapPin, Calendar, Users, Clock, Download, Phone, Mail, Info, CheckCircle, AlertCircle } from "lucide-react";
      ...
      <div className="sticky top-0 z-40 bg-white/80 backdrop-blur-md border-b border-slate-200">
        <div className="container mx-auto px-4 py-4 flex justify-between items-center">
          <div className="flex items-center gap-3">
             <div className="h-8 w-8 bg-slate-900 rounded-lg flex items-center justify-center text-lime-400 font-black text-xs">JV</div>
             <div>
                <p className="text-[10px] font-bold text-slate-400 uppercase tracking-widest">Booking Ref</p>
                <h1 className="text-lg md:text-xl font-black text-slate-900 leading-none">{booking.booking_code}</h1>
             </div>
          </div>
      ...
      <div className="container mx-auto px-4 py-8">
        <div className="mb-8">
           <h2 className="text-3xl font-bold text-slate-900">
              Hello, <span className="text-transparent bg-clip-text bg-gradient-to-r from-lime-500 to-green-600">{booking.customer_name}</span>! 👋
           </h2>
           <p className="text-slate-500 mt-2">
              Here is your personal dashboard for the <span className="font-bold text-slate-700">{booking.package_name}</span> trip.
           </p>
        </div>
        ...
        <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
            ...
            <div className="lg:col-span-2 space-y-6">
                        ...
                        <div className="bg-white rounded-2xl border border-slate-200 p-6 shadow-sm">
                            <div className="flex flex-col md:flex-row gap-4">
                                ...
                                <div className="flex-grow">
                                    {/* Text Title Biru Tua */}
                                    <h3 className="text-lg font-bold text-blue-800 mb-2">Your booking is confirmed!</h3>
                                    
                                    <div className="text-sm text-slate-600 mb-4 space-y-2">
                                        <p>
                                            Please select a payment method for your remaining balance of 
                                            <span className="font-bold text-orange-500 mx-1">{formatIDR(booking.finance.balance)}</span> 
                                            at your convenience.
                                        </p>
                                        ...
                                        {booking.finance.balance_payment_method && (
                                            <div className="bg-slate-50 p-3 rounded-lg border border-slate-100 mt-2">
                                                <p className="font-bold text-xs uppercase text-slate-400 mb-1">Selected Method</p>
                            ...
                            <div className="flex flex-col md:flex-row justify-between items-start md:items-center gap-6">
                                <div>
                                    <h3 className="text-lg font-bold text-slate-900 mb-1 flex items-center gap-2">
                                        <AlertCircle size={18} className="text-orange-500" /> Payment Required
                                    </h3>
                                    <p className="text-sm text-slate-500 mb-3 max-w-md">
                                        Please complete your payment before <b>{formatDate(booking.finance.due_date)}</b> to secure your reservation.
                                    </p>
                ...
                <div className="bg-white rounded-2xl border border-slate-200 p-6 shadow-sm">
                    ...
                    <div className="grid grid-cols-2 md:grid-cols-4 gap-y-6 gap-x-4">
                        <div>
                            <div className="flex items-center gap-2 text-lime-600 mb-1"><Calendar size={16}/> <span className="text-xs font-bold uppercase">Date</span></div>
                            <p className="font-bold text-slate-900">{formatDate(booking.travel_date_start)}</p>
                        </div>
                        <div>
                            <div className="flex items-center gap-2 text-lime-600 mb-1"><Clock size={16}/> <span className="text-xs font-bold uppercase">Duration</span></div>
                            <p className="font-bold text-slate-900">{booking.duration}</p>
                        </div>
                        <div>
                            <div className="flex items-center gap-2 text-lime-600 mb-1"><Users size={16}/> <span className="text-xs font-bold uppercase">Pax</span></div>
                            <p className="font-bold text-slate-900">{booking.total_pax} People</p>
                        </div>
                        <div>
                            <div className="flex items-center gap-2 text-lime-600 mb-1"><MapPin size={16}/> <span className="text-xs font-bold uppercase">Start</span></div>
                            <p className="font-bold text-slate-900 truncate" title={booking.pickup}>{booking.pickup || "TBA"}</p>
                        </div>
                    </div>
                </div>
                    ...
                    <div className="bg-white rounded-2xl border border-slate-200 p-6 shadow-sm">
                        ...
                        <div className="space-y-4">
                            {booking.addons.map((addon, idx) => (
                                <div key={addon.id} className="flex justify-between items-center py-2 border-b border-slate-50 last:border-0">
                                    <div className="flex items-center gap-4">
                                        ...
                                        <div>
                                            ...
                                            <p className="text-xs text-slate-500">{formatIDR(addon.price)} x {addon.qty}</p>
                                        </div>
                                    </div>
                            ...
                            <div className="flex justify-between items-center pt-4">
                                ...
                                <p className="text-xl font-black text-lime-600">{formatIDR(booking.finance.total_addons)}</p>
                            </div>
                        </div>
                    </div>
                )}
            ...
            <div className="lg:col-span-1 space-y-6">
                ...
                {/* 1. CONTACT OPERATOR */}
                <div className="bg-white rounded-2xl border border-slate-200 p-6 shadow-sm">
                    <h3 className="text-sm font-bold uppercase tracking-widest text-slate-400 mb-4">Need Help?</h3>
                    <div className="space-y-4">
                        <a href="https://wa.me/6282244788833" target="_blank" className="flex items-center gap-3 text-slate-700 hover:text-lime-600 transition-colors">
                            <div className="p-2 bg-slate-100 rounded-full"><Phone size={18}/></div>
                            <div>
                        ...
                        <div className="flex items-center gap-3 text-slate-700">
                            <div className="p-2 bg-slate-100 rounded-full"><Mail size={18}/></div>
                            <div>
                                <p className="text-xs font-bold uppercase text-slate-400">Email</p>
                                <p className="font-bold text-sm">javavolcanotouroperator@gmail.com</p>
                            </div>
                        </div>
                    </div>
                </div>
                ...
                <div className="bg-slate-900 text-white rounded-2xl p-6 shadow-lg overflow-hidden relative">
                    ...
                    <h3 className="font-bold uppercase tracking-widest mb-6 text-lime-400 text-xs relative z-10">Documents</h3>
                ...
                {booking.finance.payment_history.length > 0 && (
                    <div className="bg-white rounded-2xl border border-slate-200 p-6 shadow-sm">
                        <h3 className="text-sm font-bold uppercase tracking-widest text-slate-400 mb-4">Payment History</h3>
                        <div className="space-y-4">
                            {booking.finance.payment_history.map((hist, i) => (
                                <div key={i} className="flex justify-between items-start pb-3 border-b border-slate-50 last:border-0 last:pb-0">
                                    <div>
                                        <p className="font-bold text-slate-900 text-sm">{hist.method}</p>
                                        <p className="text-xs text-slate-400">{formatDate(hist.created_at)}</p>
                                    </div>
                                    <div className="text-right">
                                        <p className="font-bold text-slate-900 text-sm">{formatIDR(hist.nominal)}</p>
                                        <span className="text-[10px] uppercase font-bold text-lime-600 bg-lime-100 px-1.5 py-0.5 rounded">Success</span>
                                    </div>
                                </div>
                            ))}
                        </div>
