...
export const metadata: Metadata = {
    title: policyData.privacyDataProtectionPolicy.documentTitle,
    description: "Official Privacy & Data Protection Policy for Java Volcano Tour Operator (JVTO).",
};
...
const renderSectionText = (text: string) => {
    return text.split('\n').map((paragraph, pIndex) => {
        if (paragraph.trim().startsWith('-')) {
            ...
            if (pIndex > 0 && text.split('\n')[pIndex - 1].trim().startsWith('-')) return null;
            return (
                <ul key={pIndex} className="list-disc pl-5 space-y-2 my-4">
                    {items.map((item, i) => (
         ...
         if (paragraph.startsWith('4.')) {
            const subSections = paragraph.split(/\n(?=\d\.\d)/);
            return subSections.map((sub, sIndex) => {
                ...
                return (
                    <div key={sIndex} className="mt-6">
                        ...
                        <div className="prose-p:my-2">{renderSectionText(content)}</div>
                    </div>
                );
            });
        }
        if (paragraph.trim() === '') {
            return <div key={pIndex} className="py-2" />;
        }
...
export default function PrivacyPolicyPage() {
  const { privacyDataProtectionPolicy: policy } = policyData;
        ...
        <section className="py-12 md:py-16">
          <div className="container mx-auto px-4 max-w-4xl">
            ...
            <div className="prose prose-lg max-w-none mx-auto text-muted-foreground">
              <p className="border-l-4 border-primary pl-4 italic bg-accent p-4 rounded-r-lg">{policy.appliesTo}</p>

              {policy.sections.map((section, index) => (
                <section key={index} id={`section-${section.number}`} className="scroll-mt-24 mt-12">
                  <h2 className="heading-md text-2xl mb-4 text-foreground">{section.number}. {section.title}</h2>
                        ...
                        <div key={pIndex}>
                            {paragraph.split('\n').map((line, lIndex) => {
                                if (line.trim().startsWith('-')) {
                                    ...
                                    if(lIndex > 0 && paragraph.split('\n')[lIndex - 1].trim().startsWith('-')) return null;
                                    return (
                                        <ul key={lIndex} className="list-disc pl-5 space-y-2 my-4">
                                            {items.map((item, i) => <li key={i}>{item.replace(/^- /, '')}</li>)}
                                        </ul>
                                    )
                                }
                                if (/^\d\.\d/.test(line) || /^[A-Z][a-zA-Z\s]+$/.test(line) && line.toUpperCase() === line) {
                                     if(line.includes("Who We Are") || line.includes("Data We Collect")) return null;
                                    return <h3 key={lIndex} className="text-xl font-bold text-foreground mt-6 mb-2">{line}</h3>
                                }
                                return <p key={lIndex} className="my-2">{line}</p>
                            })}
                        </div>
                    ))}
                  </div>
              ...
              <div className="mt-12 border-t pt-8 text-sm italic">
                  <p>{policy.closingStatement}</p>
              </div>
            </div>
          </div>
        </section>
