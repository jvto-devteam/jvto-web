...
const travelGuideData = {
  "route": "/travel-guide",
  "seo": {
    "title": "Travel Guide – Safety, Health & Practical Information | Java Volcano Tour Operator",
    "metaDescription": "Official travel guide for private tours with Java Volcano Tour Operator (JVTO). Learn about booking, safety, Ijen health screening, weather and closures, packing, police escort for groups and ISIC student deals."
  },
  ...
  "toc": {
    ...
    "items": [
      ...
      { "id": "weather-and-closures", "label": "Weather, volcano alerts & closures" },
      { "id": "packing-and-fitness", "label": "Packing & fitness" },
      { "id": "booking-and-payments", "label": "Booking, payments & Travel Credit" },
      { "id": "police-escort-for-groups", "label": "Traffic police escort for groups" },
            ...
            <main className="flex-grow pt-24">
                <section className="py-12 md:py-16 bg-accent border-b">
                    <div className="container mx-auto px-4 max-w-4xl text-center">
                        <h1 className="heading-lg mb-6">{h1}</h1>
                        <div className="prose prose-lg mx-auto text-muted-foreground">
                            {hero.introParagraphs.map((p, i) => <p key={i}>{p}</p>)}
                        </div>
                        <div className="mt-8 flex flex-col sm:flex-row gap-4 justify-center">
                            {hero.primaryCtas.map(cta => (
                                <Button key={cta.href} size="lg" variant={cta.href.includes('tours') ? 'default' : 'outline'}>
                        ...
                        <div className="grid lg:grid-cols-12 gap-16">
                                    ...
                                    <Card>
                                        ...
                                        <CardContent>
                                            <ul className="space-y-2">
                                                {toc.items.map(item => (
                                                    <li key={item.id}>
                                                        <a href={`#${item.id}`} className="text-sm font-medium text-muted-foreground hover:text-primary transition-colors">{item.label}</a>
                                                    </li>
                                                ))}
                                            </ul>
                                        </CardContent>
                            ...
                            <main className="lg:col-span-8">
                                <Card className="bg-blue-50 border-blue-200 mb-12">
                                    <CardHeader>
                                        <CardTitle className="text-blue-800">{latestUpdate.title}</CardTitle>
                                    </CardHeader>
                                    <CardContent className="prose prose-sm max-w-none text-blue-700">
                                        <p className="font-bold">Last updated : {latestUpdate.lastUpdatedPlaceholder}</p>
                                        <ul className="list-disc pl-5">
                                            {latestUpdate.bodyParagraphs.map((p, i) => <li key={i}>{p}</li>)}
                                        </ul>
                                ...
                                <div className="space-y-12">
                                    {sections.map(section => (
                                        <section key={section.id} id={section.id} className="scroll-mt-24">
                                            <h2 className="heading-md text-2xl mb-4">{section.title}</h2>
                                            <div className="prose max-w-none text-muted-foreground">
                                                {section.summaryParagraphs.map((p, i) => {
                                                    if(p.startsWith('- ')){
                                                        const items = section.summaryParagraphs.filter(sp => sp.startsWith('- '));
                                                        if(i > 0 && section.summaryParagraphs[i-1].startsWith('- ')) return null;
                                                        return (
                                            ...
                                            {section.cta && (
                                                <div className="mt-6">
                                                    <Button variant="link" className="p-0 h-auto">
                                                        <Link href={section.cta.href}>{section.cta.label} <ArrowRight className="ml-2 w-4 h-4" /></Link>
                                                    </Button>
                                                </div>
                                            )}
                                        </section>
