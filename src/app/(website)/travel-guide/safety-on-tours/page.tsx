...
const safetyData = {
  ...
  "seo": {
    ...
    "metaDescription": "How Java Volcano Tour Operator manages safety on private tours to Bromo, Ijen and Tumpak Sewu. Police-informed planning, health screening, official MAGMA Indonesia updates and clear guest responsibilities."
  },
  ...
  "footnotes": [
    ["1", "https://esdm.lampungprov.go.id/pages/magma-indonesia-esdm-peta-informasi-dan-rekomendasi-kebencanaan-geologi?utm_source=chatgpt.com"],
    ["2", "https://www.nu.or.id/nasional/cek-gunung-api-dan-kebencanaan-download-aplikasi-magma-indonesia-TXCYW?utm_source=chatgpt.com"],
    ["3", "https://infopublik.id/kategori/nasional-sosial-budaya/856216/index.html?utm_source=chatgpt.com"]
  ]
};
...
const ParagraphRenderer = ({ text }: { text: string }) => {
    ...
    const parts = text.split(pathRegex).map((part, index) => {
        if (part && part.match(pathRegex)) {
            ...
            return <Link key={index} href={cleanPath} className="text-primary hover:underline">{cleanPath}</Link>;
        }
        ...
        const linkParts = part?.split(linkRegex).map((linkPart, linkIndex) => {
            if (linkIndex % 3 === 1) {
                ...
                return <a key={linkIndex} href={href} className="text-primary hover:underline" target="_blank" rel="noopener noreferrer">{linkText}</a>;
            }
            if (linkIndex % 3 === 0) {
                 const boldParts = linkPart?.split(boldRegex).map((boldPart, boldIndex) => {
                    if (boldIndex % 2 === 1) {
                        return <strong key={boldIndex}>{boldPart}</strong>;
                    }
                    const codeParts = boldPart.split(codeRegex).map((codePart, codeIndex) => {
                        if (codeIndex % 2 === 1) {
                            return <code key={codeIndex} className="font-mono bg-muted p-1 rounded text-sm">{codePart}</code>
                        }
                        return codePart;
                    });
                    return codeParts;
...
const renderParagraphs = (paragraphs: string[]) => {
  ...
  const flushList = () => {
    if (listItems.length > 0) {
      elements.push(
        <ul key={`list-${elements.length}`} className="list-disc pl-5 my-4 space-y-2">
  ...
  for (const p of paragraphs) {
    ...
    if (listItemMatch) {
      const indent = listItemMatch[1].trim() === '*' ? 0 : (listItemMatch[1].length - 1) / 2;
      listItems.push({ text: listItemMatch[2], indent });
    } else if (blockquoteMatch) {
      flushList();
      elements.push(
        <blockquote key={elements.length} className="border-l-4 border-muted-foreground/20 pl-4 my-4 italic">
            ...
            <main className="flex-grow pt-24">
                <section className="py-12 md:py-16 bg-accent border-b">
                    <div className="container mx-auto px-4 max-w-4xl">
                        ...
                        <h1 className="font-black text-2xl md:text-5xl text-center mb-6">{h1}</h1>
                    </div>
                </section>
