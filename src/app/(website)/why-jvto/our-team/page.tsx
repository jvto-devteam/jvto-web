...
function getCrewBio(code: string) {
  ...
  for (const section of page.sections ?? []) {
    for (const block of (section as any).blocks ?? []) {
      ...
      if (item) return item as { about?: string | null; photo_url?: string | null; highlights?: string[] };
    }
  }
  return null;
}

function CrewCard({ member }: { member: PublicCrewMember }) {
  const bio = getCrewBio(member.code);
...
export default async function OurTeamPage() {
  ...
  const extraSchemas = [buildTeamAboutPageSchema(), buildTeamItemListSchema([...guides, ...drivers])];
