...
const PropertyDisplay: React.FC<{ property: DestinationProperty }> = ({
  ...
  <div className="p-4 bg-background-light dark:bg-ink-primary rounded-lg border border-ink-neutral-200 dark:border-ink-neutral-700 h-full">
    <h4 className="font-semibold text-ink-neutral-500 dark:text-ink-neutral-400 text-sm">
      {property.name}
    </h4>
    <p className="text-lg font-bold text-ink-primary dark:text-white">
      {String(property.value)}
    </p>
    {property.description && (
      <p className="text-sm mt-1 text-ink-neutral-700 dark:text-ink-neutral-200">
        {property.description}
      </p>
    )}
  </div>
...
const Section: React.FC<{ title: string; children: React.ReactNode }> = ({
  ...
  <section className="py-8">
    <h2 className="text-2xl font-bold text-ink-primary dark:text-white border-b-2 border-primary pb-2 mb-6">
      {title}
    </h2>
    {children}
  </section>
...
const riskConfig: {
  [key in keyof RiskProfile]: { label: string; icon: string };
} = {
...
const RiskMatrix: React.FC<{ risks: RiskProfile }> = ({ risks }) => {
  const getRiskStyles = (level: RiskLevel) => {
    switch (level) {
      ...
      default:
        return "bg-ink-neutral-100 dark:bg-ink-neutral-800 text-ink-neutral-600 dark:text-ink-neutral-400";
    }
  };
    ...
    <div className="space-y-3">
      {Object.entries(risks).map(([key, level]) => {
        ...
        return (
          <div
            key={key}
            className="flex items-center gap-3 p-3 bg-white dark:bg-ink-primary rounded-lg border border-ink-neutral-200 dark:border-ink-neutral-700"
          >
            <span className="material-symbols-outlined text-primary">
              {config.icon}
            </span>
            <span className="flex-grow font-medium text-ink-primary dark:text-white">
              {config.label}
            </span>
            ...
            <span
              className={`px-3 py-1 text-xs font-bold rounded-full ${getRiskStyles(
                level as RiskLevel
              )}`}
            >
              {level as RiskLevel}
...
const DestinationDetail: React.FC = () => {
  ...
  const boundsProp = destination.properties.find(
  ...
  const relatedTours = tourPackages.filter((tour) =>
    tour.route.some((routeStop) =>
      destinationKeywords.some((keyword) =>
        routeStop.toLowerCase().includes(keyword)
      )
    )
  );
      ...
      <main className="bg-background-light dark:bg-background-dark pt-20">
        <div className="container mx-auto px-4 py-8">
          <header className="mb-8">
            ...
            <h1 className="mt-4 text-3xl md:text-5xl font-bold text-ink-primary dark:text-white">
              {destination.name}
            </h1>
            <p className="mt-3 text-lg text-ink-neutral-700 dark:text-ink-neutral-200 max-w-3xl">
          ...
          <div className="relative group/gallery mb-12">
            ...
            {destination.images.length > 1 && (
              <>
                <button
                  onClick={handlePrev}
                  className="absolute top-1/2 left-4 -translate-y-1/2 w-12 h-12 flex items-center justify-center rounded-full bg-black/40 text-white opacity-0 group-hover/gallery:opacity-100 transition-opacity hover:bg-black/60 focus:outline-none focus:ring-2 focus:ring-primary z-10"
                >
                ...
                <button
                  onClick={handleNext}
                  className="absolute top-1/2 right-4 -translate-y-1/2 w-12 h-12 flex items-center justify-center rounded-full bg-black/40 text-white opacity-0 group-hover/gallery:opacity-100 transition-opacity hover:bg-black/60 focus:outline-none focus:ring-2 focus:ring-primary z-10"
                >
                  <span className="material-symbols-outlined">
                    arrow_forward_ios
                  </span>
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
                  <PropertyDisplay
                    property={{
                      propertyID: "type",
                      name: "Type",
                      value: destination.keyInfo.type,
                    }}
                  />
                  <PropertyDisplay
                    property={{
                      propertyID: "phenomenon",
                      name: "Key Phenomenon",
                      value: destination.keyInfo.phenomenon,
                    }}
                  />
                  <PropertyDisplay
                    property={{
                      propertyID: "emotion",
                      name: "Core Emotion",
                      value: destination.keyInfo.coreEmotion,
                    }}
                  />
                  <PropertyDisplay
                    property={{
                      propertyID: "difficulty",
                      name: "Difficulty",
                      value: `${destination.keyInfo.difficulty} (${destination.keyInfo.difficultyRating})`,
                    }}
                  />
                  <PropertyDisplay
                    property={{
                      ...
                      name: "Elevation",
                      value: destination.keyInfo.elevation,
                    }}
                  />
                  <PropertyDisplay
                    property={{
                      propertyID: "bestSeason",
                      name: "Ideal Visit",
                      value: destination.keyInfo.bestSeason,
                    }}
                  />
                </div>
              </Section>
              ...
              <Section title="Trekking & Logistics">
                <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                  <PropertyDisplay
                    property={{
                      propertyID: "distance",
                      name: "Distance (Round Trip)",
                      value: destination.trekkingData.distance,
                    }}
                  />
                  <PropertyDisplay
                    property={{
                      propertyID: "duration",
                      name: "Avg. Duration",
                      value: destination.trekkingData.duration,
                    }}
                  />
                  <PropertyDisplay
                    property={{
                      propertyID: "terrain",
                      name: "Terrain",
                      value: destination.trekkingData.terrain,
                    }}
                  />
                  <PropertyDisplay
                    property={{
                      propertyID: "access",
                      name: "Access Point",
                      value: destination.trekkingData.access,
                    }}
                  />
                </div>
              </Section>
          ...
          {gpxProps.length > 0 && (
            <Section title="GPX Data & Analytics">
              <div className="grid grid-cols-2 sm:grid-cols-3 lg:grid-cols-5 gap-3 mb-6">
                {gpxProps.map((prop) => {
                  ...
                  return (
                    <div
                      key={prop.propertyID}
                      className="p-3 bg-background-light dark:bg-ink-primary rounded-lg border border-ink-neutral-200 dark:border-ink-neutral-700"
                    >
              ...
              <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
                {track && bounds && (
                  <div>
                    <h3 className="font-semibold text-ink-primary dark:text-white mb-2">
                      Route Map
                    </h3>
          ...
          {relatedTours.length > 0 && (
            <Section title={`Tours Visiting ${destination.name}`}>
              <p className="mb-6 text-ink-neutral-700 dark:text-ink-neutral-200">
                Ready to experience it for yourself? Here are our all-inclusive
                private tours that feature this incredible destination.
              </p>
