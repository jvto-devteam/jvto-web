...
import AdvancedTourFilterModal, { AdvancedFilters } from './AdvancedTourFilterModal';
...
const ToursPage: React.FC = () => {
    ...
    const handleSearch = (e: FormEvent) => {
        ...
        if (parsed.durationDays) explanationParts.push(`Duration=${parsed.durationDays}d`);
        if (parsed.destinations && parsed.destinations.length > 0) explanationParts.push(`Destinations=${parsed.destinations.join(', ')}`);
        if (parsed.private) explanationParts.push('Private=true');
        setSearchExplanation(explanationParts.length > 0 ? `Matched because: ${explanationParts.join('. ')}.` : 'Showing all tours. Try a more specific search.');
        ...
        const newFilters: AdvancedFilters = {
            ...
            experiences: parsed.destinations ? parsed.destinations.map(d => keyExperienceMap[d]).filter(Boolean) : [],
    ...
    const filteredTours = useMemo(() => {
        return tourPackages.filter((tour: TourPackage) => {
            ...
            if (filters.startCities.length > 0 && !filters.startCities.some(city => tour.originCity.toLowerCase() === city.toLowerCase())) return false;
            if (filters.endCities.length > 0 && !filters.endCities.some(city => tour.endCity.toLowerCase() === city.toLowerCase())) return false;
            if (filters.experiences.length > 0 && !filters.experiences.every(exp => tour.keyExperiences.includes(exp))) return false;
            if (filters.physicality && tour.physicality.toLowerCase() !== filters.physicality) return false;
            return true;
        });
    }, [filters]);
    ...
    const toursSectionCopy = {
        ...
        subhead: 'Browse our curated, all-inclusive tours, or use the AI search to find exactly what you\'re looking for.'
    };
            ...
            <div className="container mx-auto px-4 pt-8">
                ...
                <div className="bg-white dark:bg-ink-primary p-6 rounded-2xl shadow-card my-8">
                    <form onSubmit={handleSearch}>
                        <label htmlFor="ai-search" className="block text-lg font-semibold mb-2 text-ink-primary dark:text-white">
                        ...
                        <div className="flex flex-col sm:flex-row gap-2">
                            ...
                            />
                            <button type="submit" className="px-6 py-3 rounded-lg bg-primary text-white font-semibold hover:bg-opacity-90 transition-colors flex-shrink-0">
                                Search
                            </button>
                        </div>
                        {searchExplanation && <p className="text-sm text-ink-neutral-500 dark:text-ink-neutral-400 mt-2 italic">{searchExplanation}</p>}
                    </form>
                </div>
            </div>
