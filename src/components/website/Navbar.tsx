...
const Navbar: React.FC = () => {
  ...
  const finalMenuIconClass = isMenuOpen ? "text-jvto-dark" : (isHome && !isScrolled ? "text-white" : "text-jvto-dark");

  return (
    <nav className={`fixed top-0 left-0 right-0 z-50 transition-all duration-300 ${navClass}`}>
      {/* Top Utility Bar - Only visible when not scrolled on Home, or hidden on mobile */}
      <div className={`hidden md:block py-2 text-xs font-medium border-b border-white/10 ${isHome && !isScrolled ? 'bg-black/20' : 'bg-jvto-green text-jvto-dark'}`}>
        <div className="container mx-auto px-6 flex justify-center items-center gap-4">
          ...
          <span className="hidden lg:inline">Licensed East Java Operator (No. 1102230032918)</span>
        </div>
      </div>
      ...
      <div className="container mx-auto px-6 h-20 flex items-center justify-between">
        ...
        <div className="flex items-center gap-6">
          ...
          <div className="hidden lg:flex items-center gap-8 font-bold text-sm uppercase tracking-wider">
            <Link href="/tours" className="hover:text-jvto-green transition-colors">Private Tours</Link>
            <Link href="/destinations" className="hover:text-jvto-green transition-colors">Destinations</Link>
            <Link href="/why-jvto" className="hover:text-jvto-green transition-colors">Why JVTO</Link>
            <Link href="/travel-guide" className="hover:text-jvto-green transition-colors">Travel Guide</Link>
          </div>
        </div>

        {/* Center: Logo */}
        <div className="absolute left-1/2 transform -translate-x-1/2 text-center z-50">
          <Link href="/" className={`text-2xl font-black italic tracking-tighter flex items-center gap-1 ${finalLogoTextClass}`}>
            <span className="text-3xl">JVTO</span>
          </Link>
        </div>
      ...
      {isMenuOpen && (
        <div className="fixed inset-0 bg-white z-40 pt-24 px-6 lg:hidden flex flex-col animate-fade-in-up h-screen overflow-y-auto">
          <div className="flex flex-col gap-6 text-xl font-bold uppercase tracking-wide text-jvto-dark">
            <Link href="/tours" className="border-b border-gray-100 pb-4 hover:text-jvto-green transition-colors">Private Tours</Link>
            <Link href="/destinations" className="border-b border-gray-100 pb-4 hover:text-jvto-green transition-colors">Destinations</Link>
            <Link href="/why-jvto" className="border-b border-gray-100 pb-4 hover:text-jvto-green transition-colors">Why JVTO</Link>
            <Link href="/travel-guide" className="border-b border-gray-100 pb-4 hover:text-jvto-green transition-colors">Travel Guide</Link>
            <Link href="/contact" className="border-b border-gray-100 pb-4 hover:text-jvto-green text-jvto-green transition-colors">Book Now</Link>
          </div>
