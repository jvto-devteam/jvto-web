...
const FilterDrawer = ({ 
  ...
  return (
    <>
      ...
      {/* Drawer Panel */}
      <div className={`fixed top-0 right-0 h-full w-full max-w-[350px] bg-white z-50 shadow-2xl transform transition-transform duration-300 ease-in-out flex flex-col ${
          ...
          <div>
            ...
            <div className="bg-gray-50 p-4 rounded-lg border border-gray-100">
              <div className="flex justify-between text-sm text-gray-500 mb-2">
                <span>0</span>
                <span className="font-bold text-jvto-dark">{formatIDR(filters.maxPrice)}</span>
              </div>
              <input
                type="range"
                min={0}
...
const FeaturedToursClient = ({
  ...
  const [activeLocation, setActiveLocation] = useState<"surabaya" | "bali">("surabaya");
  ...
  const filteredData = useMemo(() => {
    ...
    return baseData.filter(tour => {
      // 1. Duration Check
      if (filters.duration && tour.duration.day !== filters.duration) return false;
      ...
      if (filters.categories.length > 0) {
        ...
        const hasMatch = filters.categories.some(cat => {
          if (cat === "Volcano") return tourTags.includes("volcano") || tourTags.includes("bromo") || tourTags.includes("ijen");
          if (cat === "Waterfall") return tourTags.includes("waterfall") || tourTags.includes("madakaripura") || tourTags.includes("tumpak-sewu");
          if (cat === "Beach") return tourTags.includes("beach") || tourTags.includes("papuma") || tourTags.includes("papuma-beach");
          if (cat === "Wildlife") return tourTags.includes("wildlife") || tourTags.includes("taman-safari");
          return false;
        });
  ...
  const activeFilterCount = (filters.duration ? 1 : 0) + (filters.categories.length) + (filters.maxPrice < globalMaxPrice ? 1 : 0);
    ...
    <div className="container mx-auto px-6">
      <div className="text-center mb-8">
        ...
        <p className="text-gray-600 max-w-2xl mx-auto mb-8">
          ...
          transport, and Ijen health screening so you can focus on the adventure.
        </p>
      ...
      {filteredData.length > 0 ? (
        ...
        <div className="text-center py-16 bg-gray-50 rounded-xl border border-dashed border-gray-300 mx-auto max-w-2xl">
          ...
          <h3 className="text-lg font-bold text-gray-900 mb-2">No matching tours found</h3>
          <p className="text-gray-500 mb-6">
            Try adjusting your filters or search criteria.
          </p>
          <button
            onClick={() => setFilters({ duration: null, maxPrice: globalMaxPrice, categories: [] })}
