    ...
    <div className="group flex flex-col h-full bg-white shadow-sm hover:shadow-md transition-shadow duration-300 relative rounded-sm overflow-hidden">
      ...
      <div className="relative overflow-hidden aspect-[4/3]">
        ...
        <div className="absolute top-4 left-4 flex flex-col gap-2 items-start">
          ...
          <span className={`${difficultyColors[tour.difficulty]} text-white text-[10px] font-bold uppercase px-2 py-1 tracking-wider rounded-sm shadow-sm backdrop-blur-sm bg-opacity-90`}>
            {tour.difficulty}
          </span>
        </div>
        ...
        <div className="absolute top-4 right-4 flex flex-col gap-1 items-end">
           {tour.tags.slice(0, 2).map((tag, i) => (
             <span key={i} className="bg-white/90 backdrop-blur-sm text-jvto-dark text-[10px] font-bold uppercase px-2 py-1 tracking-wider rounded-sm shadow-sm">
               {tag}
             </span>
           ))}
        </div>
        ...
        <div className="absolute bottom-0 left-0 w-full p-4 bg-gradient-to-t from-black/80 to-transparent flex items-end">
           <div className="flex items-center gap-1 text-white">
             ...
             <span className="text-xs text-gray-300">({tour.reviewCount} reviews)</span>
           </div>
        </div>
      </div>
      ...
      <div className="flex flex-col flex-grow pt-4 pb-4 px-4 border-x border-b border-gray-100 rounded-b-sm">
        <div className="flex items-center gap-2 text-xs text-gray-500 mb-2 uppercase tracking-wide font-medium">
          ...
          Start: <span className="text-gray-700 font-bold">{tour.originCity}</span>
        </div>
        
        <h3 className="font-bold text-lg leading-tight mb-3 group-hover:text-jvto-green transition-colors line-clamp-2 min-h-[3.5rem]">
          {tour.name}
        </h3>
        ...
        <div className="mt-auto space-y-3 pt-4 border-t border-gray-100">
          <div className="flex items-center justify-between">
            <div className="flex flex-col">
              <span className="text-[10px] text-gray-400 uppercase font-bold tracking-wide">Starting From</span>
