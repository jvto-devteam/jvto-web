...
const PillarCard: React.FC<PillarCardProps> = ({ pillar, onCtaClick }) => {
  const ctaId = `pillars_${pillar.title.toLowerCase().split(' ')[0]}`;
  return (
    <div className="group flex flex-col items-center text-center p-6 rounded-2xl bg-white dark:bg-background-dark shadow-card hover:shadow-cardHover transition-all duration-300 transform hover:-translate-y-1 h-full">
        ...
        <Link 
            to={pillar.link} 
            onClick={() => onCtaClick(ctaId, pillar.cta, 'pillars')}
...
interface PillarsOfTrustProps {
    onCtaClick: (ctaId: string, ctaText: string, section: string) => void;
}

const PillarsOfTrust: React.FC<PillarsOfTrustProps> = ({ onCtaClick }) => {
  return (
    <section className="py-16 md:py-28 bg-background-light dark:bg-ink-primary">
      <div className="container mx-auto px-4">
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-8 max-w-6xl mx-auto">
          {pillars.map((pillar) => (
            <PillarCard key={pillar.title} pillar={pillar} onCtaClick={onCtaClick} />
          ))}
        </div>
      </div>
    </section>
